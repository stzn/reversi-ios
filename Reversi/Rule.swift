...
enum Rule {
    ...
    static func flippedDiskCoordinatesByPlacingDisk(
        _ disk: Disk, atX x: Int, y: Int, on disks: [DiskPosition: Disk]
    ) -> [(Int, Int)] {
        let directions = [
            (x: -1, y: -1),
            (x: 0, y: -1),
        ...
        guard disks[.init(x: x, y: y)] == nil else {
            return []
        }
        ...
        for direction in directions {
            ...
            flipping: while true {
                x += direction.x
                y += direction.y
                switch (disk, disks[.init(x: x, y: y)]) {
    ...
    static func canPlaceDisk(_ disk: Disk, atX x: Int, y: Int, on board: Board) -> Bool {
        return !flippedDiskCoordinatesByPlacingDisk(disk, atX: x, y: y, on: board.disks).isEmpty
    }
